...
#[derive(Default)]
struct MJSectionVisitor;

impl<'de> Visitor<'de> for MJSectionVisitor {
    type Value = MJSection;
    ...
    where
        M: MapAccess<'de>,
    {
        let mut result = MJSection::default();
...
impl<'de> Deserialize<'de> for MJSection {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        deserializer.deserialize_map(MJSectionVisitor::default())
    }
}
...
mod tests {
    ...
    fn serialize() {
        ...
        assert_eq!(
            ...
            r#"{"type":"mj-section","attributes":{"margin":"42px"},"children":[]}"#
        );
    }
