...
#[derive(Default)]
struct MJColumnVisitor;

impl<'de> Visitor<'de> for MJColumnVisitor {
    type Value = MJColumn;
    ...
    where
        M: MapAccess<'de>,
    {
        let mut result = MJColumn::default();
...
impl<'de> Deserialize<'de> for MJColumn {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        deserializer.deserialize_map(MJColumnVisitor::default())
    }
}
...
mod tests {
    ...
    fn serialize() {
        ...
        assert_eq!(
            ...
            r#"{"type":"mj-column","attributes":{"margin":"42px"},"children":[]}"#
        );
    }
